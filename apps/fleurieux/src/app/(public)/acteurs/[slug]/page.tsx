    ...
    <>
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm text-village-600 dark:text-village-400">{acteur.categorie.emoji} {acteur.categorie.nom}</p>
            <h1 className="mt-1 text-3xl font-bold text-gray-900 dark:text-gray-100">
              {acteur.emoji} {acteur.nom}
            </h1>
            {acteur.noteAverage && acteur.nbAvis > 0 && (
              <p className="mt-1 text-amber-500">
                {formatEtoiles(acteur.noteAverage)} <span className="text-sm text-gray-500">({acteur.nbAvis} avis)</span>
              </p>
            )}
          </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
                  ...
                  {acteur.avis.map(avis => (
                    <div key={avis.id} className="rounded-lg border border-gray-100 p-3 dark:border-gray-800">
                      <div className="flex items-center gap-2">
                        <span className="text-amber-400 text-sm">{formatEtoiles(avis.note)}</span>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-xl border border-gray-200 p-4 dark:border-gray-700 space-y-2">
              {acteur.adresse && <p className="text-sm text-gray-600 dark:text-gray-400">📍 {acteur.adresse}, {acteur.codePostal} {acteur.ville}</p>}
              {acteur.telephone && (
                <p className="text-sm">
                  <a href={`tel:${acteur.telephone}`} className="text-village-600 hover:underline dark:text-village-400">📞 {acteur.telephone}</a>
                </p>
              )}
              {acteur.email && (
                <p className="text-sm">
                  <a href={`mailto:${acteur.email}`} className="text-village-600 hover:underline dark:text-village-400">✉️ {acteur.email}</a>
                </p>
              )}
              {acteur.siteWeb && (
                <p className="text-sm">
                  <a href={acteur.siteWeb} target="_blank" rel="noopener noreferrer" className="text-village-600 hover:underline dark:text-village-400">🌐 Site web</a>
                </p>
              )}
              {/* Paiement */}
