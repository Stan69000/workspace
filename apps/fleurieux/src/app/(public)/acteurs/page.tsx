  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-gray-500 dark:text-gray-400">{acteurs.length} acteur{acteurs.length > 1 ? 's' : ''} référencé{acteurs.length > 1 ? 's' : ''}</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          <a href="/acteurs" className={`rounded-full px-3 py-1 text-sm font-medium transition-colors ${!categorie ? 'bg-village-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300'}`}>
            Tous
          </a>
          {categories.map(cat => (
            <a
              ...
              className={`rounded-full px-3 py-1 text-sm font-medium transition-colors ${categorie === cat.slug ? 'bg-village-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-800 dark:text-gray-300'}`}
            >
              {cat.emoji} {cat.nom}
            </a>
          ))}
        ...
        <div className="flex gap-1 rounded-lg border border-gray-200 p-0.5 dark:border-gray-700">
          <a href={`/acteurs?${new URLSearchParams({ q, categorie, vue: 'liste' })}`}
            className={`rounded px-3 py-1 text-sm ${vue !== 'carte' ? 'bg-village-600 text-white' : 'text-gray-600 dark:text-gray-400'}`}>
            Liste
          </a>
          <a href={`/acteurs?${new URLSearchParams({ q, categorie, vue: 'carte' })}`}
            className={`rounded px-3 py-1 text-sm ${vue === 'carte' ? 'bg-village-600 text-white' : 'text-gray-600 dark:text-gray-400'}`}>
            Carte
          </a>
        </div>
