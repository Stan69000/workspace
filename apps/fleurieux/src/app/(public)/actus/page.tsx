...
export default async function ActusPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {actus.length === 0 ? (
        <p className="py-16 text-center text-gray-400">Aucune actualité pour l&apos;instant.</p>
      ) : (
            ...
            <Card key={actu.id} hover>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  {actu.source && (
                    <p className="text-xs font-medium uppercase tracking-wide text-village-600 dark:text-village-400">{actu.source}</p>
                  )}
                ...
                {actu.publishedAt && (
                  <p className="shrink-0 text-xs text-gray-400">{formatDate(actu.publishedAt)}</p>
                )}
              </div>
            </Card>
          ))}
