    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-1 text-gray-500 dark:text-gray-400">{evenements.length} événement{evenements.length > 1 ? 's' : ''} à venir</p>
        </div>
        <a
      ...
      {evenements.length === 0 ? (
        <p className="py-16 text-center text-gray-400">Aucun événement à venir pour l&apos;instant.</p>
      ) : (
              ...
              <div className="flex-1">
                ...
                {evt.lienInscription && (
                  <a href={evt.lienInscription} target="_blank" rel="noopener noreferrer"
                    className="mt-2 inline-block text-xs font-medium text-village-600 hover:underline dark:text-village-400">
                    S&apos;inscrire →
                  </a>
                )}
              </div>
